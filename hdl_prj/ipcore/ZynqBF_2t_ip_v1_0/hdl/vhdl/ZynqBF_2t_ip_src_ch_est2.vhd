...
ARCHITECTURE rtl OF ZynqBF_2t_ip_src_ch_est2 IS
  ...
  signal dp_done_reg                        : unsigned(0 to 1);
  ...
  signal rxi_d1, rxi_d2                     : std_logic_vector(15 downto 0);
  signal rxq_d1, rxq_d2                     : std_logic_vector(15 downto 0);
...
BEGIN
    ...
    gs_addr_msb <= std_logic_vector(dp_count(11 downto 5));
    gs_addr_lsb <= std_logic_vector(dp_count(4 downto 0));
    dp_start <= '1' when cs_est = s_wait and en = '1' else '0';
    dp_en <= '1' when cs_est = s_dp else '0';
    dp_last <= '1' when cs_est = s_dp and dp_count_dreg(3) = to_unsigned(16#0fff#, dp_count'high+1) else '0';
    ...
    begin
        if clk'event and clk = '1' then
            if reset = '1' then
                ...
                if en = '1' and rx_addr_i >= base_addr_i then
                    dp_count <= dp_count + 1;
                else
                    dp_count <= (others => '0');
                end if;
    ...
    begin
        if clk'event and clk = '1' then
            if reset = '1' then
                ...
                if dp_done = '1' then
                    ch_i <= std_logic_vector(signed(macc_ac) + signed(macc_ad));
                    ch_q <= std_logic_vector(signed(macc_ad) - signed(macc_ac));
                end if;
            end if;
        end if;
    end process;
